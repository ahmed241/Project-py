...
try:
    ...
    print("[ERROR] Error: Could not import PSG_data_EOT.py. Make sure the file is in the same directory.", file=sys.stderr)
    sys.exit(1)
...
def design_eot_crane(load_tonnes, lift_height, hoist_speed):
    ...
    results = {}
    print("-" * 50)
    print(f"STARTING EOT CRANE DESIGN CALCULATION")
    print(f"INPUTS: Load={load_tonnes:.2f} T, Lift Height={lift_height:.2f} m, Hoist Speed={hoist_speed:.2f} m/min")
    print("-" * 50)
    ...
    if suitable_ropes.empty:
        print(f'[ERROR] Error: No suitable rope found in PSG data for a breaking load of {breaking_load:.2f} Tonnes.')
        return None
    ...
    if c1_row.empty:
        print(f"[ERROR] Error: Could not find a C1 value for rope diameter {selected_rope_d:.2f} mm. Design cannot proceed.")
        return None
    ...
    if suitable_pulleys.empty:
        print(f"[ERROR] Error: No suitable pulley found for rope diameter {selected_rope_d:.2f} mm.")
        return None
    ...
    results['Pulley'] = {'SelectedPulley': chosen_pulley, 'PulleyDiameter_mm': pulley_dia}
    ...
    axle_dia_cal = np.cbrt(bending_moment / 10.79922475) 
    
    suitable_axles = psg.bearing_data[psg.bearing_data['d'] >= axle_dia_cal]
    if suitable_axles.empty:
        print(f"[ERROR] Error: No standard bearing found for a required axle diameter of {axle_dia_cal:.2f} mm.")
        return None
    axle_dia = suitable_axles['d'].iloc[0] 
    ...
    if valid_bearings.empty:
        print(f"[ERROR] Error: No bearing found with diameter {axle_dia:.0f} mm that can support a dynamic load of {dynamic_load:.2f} kgf.")
        return None
    ...
    results['AxleAndBearing'] = {'FinalAxleDiameter_mm': axle_dia, 'SelectedBearing': chosen_bearing, 'DynamicLoad_kgf': dynamic_load}
    ...
    if selected_hook.empty:
        print(f"[ERROR] Error: No standard hook found for a safe load of {load_tonnes:.2f} Tonnes.")
        return None
    ...
    results['Hook'] = {'ChosenHook': chosen_hook, 'ResultantStress_N_mm2': resultant_stress, 'Status': hook_status}
    ...
    if chosen_nut.empty:
        print(f"[ERROR] Error: No thread data found for nut nominal diameter {chosen_hook['G1_value']} mm.")
        return None
    ...
    results['Nut'] = {'ChosenNut': chosen_nut.iloc[0], 'NumberOfThreads': n_thread, 'NutHeight_mm': h_nut}
    ...
    if valid_thrust_bearings.empty:
        print(f"[ERROR] Error: No thrust bearing found for static load {static_load_kgf:.0f} kgf and min diameter {chosen_hook['Gmin']} mm.")
        return None
    ...
    results['ThrustBearing'] = {'ChosenBearing': chosen_thrust_bearing}
    ...
    if selected_drum_groove.empty:
        print(f"[ERROR] Error: No drum groove data found for rope diameter {selected_rope_d:.2f} mm.")
        return None
    ...
    udl = drum_wt_N / (Ld_mm / 1000) 
    
    Mb_shaft = (load_per_fall * 50000 * (Ld_cm/100)) - (udl * (Ld_cm/100)**2 * 0.125) 
    torque = 1000 * ((18.5 * 60000) / (2 * math.pi * pulley_rpm)) 
    
    equi_torque = math.sqrt((Mb_shaft*1000)**2 + torque**2) 
    d_shaft = math.ceil(np.cbrt(equi_torque / (0.196 * 114))) 
    ...
    print("-" * 50)
    print(f"DESIGN CALCULATION FINISHED")
    print("-" * 50)
    return results